...
SHELL            := /bin/bash
PROJECT          := github.com/MaienM/pulumi-radarr
NODE_MODULE_NAME := @maienm/pulumi-radarr
TF_NAME          := radarr
...
TFGEN            := pulumi-tfgen-radarr
PROVIDER         := pulumi-resource-radarr
...
upstream/.git:
	...
	git -c advice.detachedHead=false clone --depth 1 --branch 'v$(PROVIDER_VERSION)' 'https://github.com/devopsarr/terraform-provider-radarr' upstream
